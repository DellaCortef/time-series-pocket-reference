...
Se examinarmos a documentação da função *ar()* (*https://perma.cc/8H8Z-CX9R*), podemos ver que a ordem selecionada é determinada (com parâmetros padrã0 que não alteramos) mostra que a seleção visual que fizemos examinando PACF é compatível com a seleção que seria feita minimizando um critério de informação. Apesar de serem duas formas diferentes de escolher a ordem do modelo, aqui elas são compatíveis.

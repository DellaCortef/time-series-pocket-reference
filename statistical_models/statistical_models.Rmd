...
The following table shows the selected lags based on different criteria.
...
The following table displays the values of the AIC, HQ, SC, and FPE criteria for different lag lengths.

```{r, echo=FALSE}
# Load knitr for table formatting
library(knitr)

# Create a data frame for the criteria table
criteria <- data.frame(
  Lag = c(1, 2, 3, 4),
  `AIC(n)` = c(3.975854e+01, 3.967373e+01, 3.957496e+01, 3.968281e+01),
  `HQ(n)` = c(3.984267e+01, 3.981395e+01, 3.977126e+01, 3.993521e+01),
  `SC(n)` = c(3.997554e+01, 4.003540e+01, 4.008130e+01, 4.033382e+01),
  `FPE(n)` = c(1.849280e+17, 1.700189e+17, 1.542863e+17, 1.723729e+17)
)

# Display the criteria table
kable(criteria, caption = "VAR Model Criteria for Different Lags")
```
...
\[
\text{Banking.orders..2.} = \text{Banking.orders..2..l1} + \text{Banking.orders..3..l1} + \text{Banking.orders..2..l2} + \text{Banking.orders..3..l2} + \text{Banking.orders..2..l3} + \text{Banking.orders..3..l3} + \text{const}
\]

##### - Coefficients Table for `Banking.orders..2.`
...
coefficients_2 <- data.frame(
  Term = c("Banking.orders..2..l1", "Banking.orders..3..l1", "Banking.orders..2..l2",
           "Banking.orders..3..l2", "Banking.orders..2..l3", "Banking.orders..3..l3", "const"),
  Estimate = c(-2.134351e-01, -1.070454e+00, -3.332587e-02, 6.778114e-01, -2.378322e-01, 8.626345e-01, 1.055717e+05)
)

# Display the coefficients table
kable(coefficients_2, col.names = c("Term", "Estimate"), caption = "Estimated Coefficients for Banking.orders..2.")
```

##### Estimated Coefficients for Banking.orders..3.

| Term                   | Estimate       |
|------------------------|----------------|
| Banking.orders..2..l1  | 0.0998519500   |
| Banking.orders..3..l1  | 0.4954869000   |
| Banking.orders..2..l2  | 0.0381905300   |
| Banking.orders..3..l2  | 0.4288644000   |
| Banking.orders..2..l3  | -0.004107863   |
| Banking.orders..3..l3  | -0.268197900   |
| const                  | -2320.958000   |


We can observe that the function allows a variety of information criteria to be chosen. Also note that we indicated that we wanted to adjust a "*const*" term to accommodate a non-zero mean. We could also choose to fit a *drift* term, both, or neither, but the "*const*" option seems to be the best for our data. 
We will analyze three lags:

![Demand banking adjusted model](https://github.com/DellaCortef/time-series-pocket-reference/blob/main/statistical_models_for_time_series/images/demand-banking-adjusted-model.R.png?raw=true)

In the upper part of the figure, we observe the actual values (solid line) versus. predicted values (dashed line) for bank orders (2) and, at the bottom, we observe the same for bank orders (3). Interestingly, the top graph looks more like a typical forecast, where the forecast is a little "slow" in changing relative to the actual data, whereas in the bottom graph we see that the forecast predicted changes a little before they happened. This suggests that bank orders (2) "lead" bank orders (3), which means that bank orders (2) are useful in predicting traffic controller orders, but not vice versa.

![Demand banking ACFs](https://github.com/DellaCortef/time-series-pocket-reference/blob/main/statistical_models_for_time_series/images/demand-banking-acf.R.png?raw=true)

We plot the autocorrelation function of the residuals for each **time series**. Note that both **time series** have a significant autocorrelation of errors at lag 3, which cannot be fully accounted for in the model.

<br>
It is not clear whether the ACF is in favor of the absence of autocorrelation in the residuals as we would like. Thus, we can also apply the *Postmanteau* test for serial correlation through the *serial.test()* method of the **vars** package. It is a test similar to the correlation tests we saw in the univariate case:


##### Portmanteau Test (Asymptotic)

The Portmanteau test was conducted on the residuals of the VAR model (`est.var`).
...
The test resulted in a Chi-squared statistic of 20.463 with 20 degrees of freedom, yielding a p-value of 0.4293. Based on this p-value, there is insufficient evidence to reject the null hypothesis, suggesting that the residuals of the VAR model may not exhibit significant autocorrelation.

As the *p*-value is very high, we cannot reject the null hypothesis that there is no serial correlation in the residuals. This gives us more evidence that the model does an acceptable job.

Since for univariate models we examined different models, resulting in the ARMA and ARIMA versions, it is possible that VARIMA exists, but it is not widely used, as VAR has relatively good performance and is quite complicated. In practice, for corporate and academic use cases, we will see the predominant use of VAR instead of VARIMA.

Another class of model is CVAR, cointegrated vector autoregression. Refers to the case where individual **time series** are not stationary, but a linear combination of stationary **time series** without differentiation.


## Variations in Statistical Models

There are many other types of statistical models developed for **time series** data. Some of them extend the ARIMA model, while others make different underlying assumptions about the temporal dynamics used in the ARIMA model.

### Seasonal Arima

A seasonal ARIMA (SARIMA) model assumes multiplicative seasonality. For this reason, the SARIMA model can be expressed as ARIMA (*p, d, q*) x (*P, D, Q*)*m*. The model states that seasonal behavior itself can be thought of as an ARIMA process, with *m* specifying the number of time intervals per seasonal cycle. The important thing about this factorization is that the model recognizes which adjacent points in time can influence each other, whether in the same seasonality or in different seasonalities, through the usual temporal proximity methods.

Identifying a SARIMA model is even more complicated than identifying an ARIMA model because it is necessary to deal with seasonal effects. Fortunately, *auto.arima()* in the **forecast** package can handle this the same way it handles a standard ARIMA estimation task. As discussed, there are good reasons to choose automatic parameter selection, unless you have strong knowledge that suggests replacing the selected model with automated methods.

### ARCH, GARCH and their analogues

ARCH stands for "autoregressive conditional heteroskedasticity". This model is used almost exclusively in the financial sector. It is often covered in **time series** courses, so it is worth studying. This class of models is based on the observation that stock prices do not have constant variance, and that, in fact, the variance itself appears conditional autoregressive when compared to previous variances (for example, days of high volatility on the stock market come into groups). In these models, the variance of a process is modeled as an autoregressive process and not the process itself.

### Hierarchical time series models

Hierarchical **time series** are quite common in the real world, although they are not considered as such. You can easily imagine situations in which they appear:
- total monthly demand in dollars for a company's products, which can be summarized in the SKU number (inventory control unit);
weekly political polling data from the electorate, and then the same polling data divided by demographics (overlapping or not), such as women versus men or SP state versus RJ;
- total number of tourists who disembark daily in the EU compared to the total number of tourists who disembark in each member country;

### Hierarchical Time Series ModelsA convenient way to deal with hierarchical **time series** is through the **hts** R package. You can use this package to visualize hierarchical **time series** data and generate predictions. Forecasts can be generated with a series of different methodologies traditionally available in this package:

- generate low-level forecasts (more individualized) and aggregate them to generate higher-level forecasts;
- generate high-level forecasts and then generate lower-level forecasts based on the historical proportions of each component of the aggregate. This methodology tends to be less accurate when it comes to low-level forecasts, although this can be mitigated somewhat through a variety of techniques that predict how aggregate proportions will change over time;

Hierarchical **time series** are quite common in the real world, although they are not considered as such. You can easily imagine situations in which they appear:
- total monthly demand in dollars for a company's products, which can be summarized in the SKU number (inventory control unit);
weekly political polling data from the electorate, and then the same polling data divided by demographics (overlapping or not), such as women versus men or SP state versus RJ;
- total number of tourists who disembark daily in the EU compared to the total number of tourists who disembark in each member country;
- it is possible to try to get the best of each methodology by choosing an "intermediate" approach in which a mid-level forecast is generated (assuming we have several layers of hierarchy). So, just propagate your predictions in an ascending and descending manner to make other predictions.

Finally, the standard of excellence of the **hts** package allows that, at all hierarchical levels, forecasts can be aggregated independently of other hierarchy levels. Thus, **hts** combines these predictions to ensure consistency in one method, according to Hyndman et al (https://perma.cc/G4eG-6SMP).

Many of the statistical models we analyzed would, in turn, be applied to the problem of forecasting whatever **time series** level was established. The hierarchical part of the analysis is often a wrapper around these fundamental models.

## Advantages and Disadvantages of Statistical Methods for Time Series

<u>*Advantages*</u>

- these templates are simple and transparent. Thus, its parameters can be clearly understood;
- because of the simple mathematical expressions that define these models, it is possible to derive their properties of interest in a statistically precise way;
- we can apply these models to relatively small data sets and still obtain good results;
- these simple models and related modifications perform excellently even compared to complicated machine learning models. This way, we obtain good performance without the risk of overfitting;
- automated and well-developed methodologies for choosing the orders of your models and estimating their parameters simplify the generation of these predictions;


<u>*Disadvantages*</u>

- as these models are quite simple, they do not always improve performance when dealing with large data sets. If you are working with gigantic data sets, you may be better off using complex machine learning models and neural network methodologies;
- these statistical models focus on point estimates of the mean value of a distribution, not the distribution. Yes, we can derive sample variances and the like as a proxy for uncertainty in predictions, however, the fundamental model only allows for limited ways of expressing uncertainty regarding all the choices we make when selecting the model;
- by definition, these models are not created to deal with non-linear dynamics, leaving something to be desired when representing data where non-linear relationships are dominant;